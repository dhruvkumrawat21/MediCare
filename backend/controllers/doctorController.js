...
//this docter creation willl be done by the admin panel  


//Helper Functions
//this below function will convert time 12 hrs to number of minutes since midnight
//and also return am or pm accordingluly
...
//this below function is for removing the duplicate slot and return the slot after filtering by using the tume 
...
//this function accet the object or json string 
...
//this will convert the doctor data to the plain text (as data coming from doctor schema is not a plain data)
...
//to create a Doctor
//this docter creation will be done by the admin panel 
export async function createDoctor(req, res) {
    try {
        ...
        console.error("CreatedDoctor error: ", err);
        return res.status(500).json({
            success: false,
...
export const getDoctors = async (req, res) => {
    try {
        const { q = "", limit: limitRaw = 200, page: pageRaw = 1 } = req.query;     // limit per page is set to the 200 dr. per page
        const limit = Math.min(500, Math.max(1, parseInt(limitRaw, 10) || 200));
        const page = Math.max(1, parseInt(pageRaw, 10) || 1);
        const skip = (page - 1) * limit;    //if limit exceeded go to the next page

        const match = {};
        if (q && typeof q === "string" && q.trim()) {
            const re = new RegExp(q.trim(), "i");
            match.$or = [{ name: re }, { specialization: re }, { speciality: re }, { email: re }];  //filtering the doctor by specilization speciality and email 
        }
        ...
        //normalizing the data and fetching the data for the specific doctor 
        ...
        const total = await Doctor.countDocuments(match);   //total no of doctors present in db
...
//to get doctor by id  that is to fetch one doctor
...
export async function updateDoctor(req, res) {
    try {
        ...
        //if  doctor exists and we want to update the image of the doctor
        //then previous one should be deleted from the cloudinary 
        //and then update in the cloudinary 
        if (req.file?.path) {
            const uploaded = await uploadToCloudinary(req.file.path, "Doctor");
            if (uploaded) {
        ...
        delete out.password;
        return res.json({ success: true, data: out });  //updated data will return here
    } catch (err) {
...
//to toggle availability of the doctor 
...
export async function toggleAvailability(req, res) {
    try {
        ...
        if (!req.doctor || String(req.doctor._id || req.doctor.id) !== String(id)) {
            return res.status(403).json({ success: false, message: "Not authorized to toggle the availability of the doctor" });
        }
        ...
        console.error("toToggleAvailabilityDoctor error:", err);
        return res.status(500).json({ success: false, message: "Server error" });
    }
}

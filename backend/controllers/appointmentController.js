...
//this is used for returning a a finite number.
const safeNumber = (v) => {
    const n = Number(v);
    return Number.isFinite(n) ? n : null;
};

//this will create a frontend url
...
//this will make sure that the user is coming from the clerk
...
export const getAppointmentByPatient = async (req, res) => {
    try {
        ...
        const resolvedCreatedBy = queryCreatedBy || clerkUserId || null;//priority decide krega jo id aa jae phele vo le lega 
        //ydi queryCreatedBy h to chi aa jaega resolved createdBy me vrna clerkUserId or dono nhi rhe to null aa jaega 

        console.log(`resolvedCreatedBY (query or req.auth.userId) : `, resolvedCreatedBy);

        if (!resolvedCreatedBy && !req.query.mobile) {
            return res.status(401).json({   //401 means unauthorized person
                success: false,
...
export const createAppointment = async (req, res) => {
    try {
        ...
        if (numericFee === null || numericFee < 0) {
            return res.status(400).json({
                success: false,
                message: "Fee must be valid number"
            });
        }
        ...
        if (existingBooking) {
            return res.status(409).json({
                success: false,
                message: "You have already amd appointment with this doctor at the selected slot."
            });
        }
        ...
        try {
            ...
            console.warn("Doctor lookip failed : ", e?.message || e);
        }
...
export const confirmPayment = async (req, res) => {
    try {
        ...
        try {
            ...
            console.error("Stripe retrive session error:", err);
...
export const updateAppointment = async (req, res) => {
    try {
        ...
        console.error("UpdatedAppointment Error:", error);
        return res.status(500).json({
            success: false,
            message: "Server error"
        })
...
export const getStats = async (req, res) => {
    try {
        ...
        console.error("GetStatsAppointment Error:", error);
        return res.status(500).json({
            success: false,
            message: "Server error"
        })
...
export async function getRegisterUserCount(req, res) {
    try {
        ...
        console.error("getRegisterUserCountr Error:", error);
        return res.status(500).json({
            success: false,
            message: "Server error"
        })

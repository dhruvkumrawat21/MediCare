...
const stripeKey = process.env.STRIPE_SECRET_KEY || process.env.STRIPE_KEY || null;
const stripe = stripeKey ? new Stripe(stripeKey, { apiVersion: "2023-10-16" }) : null;


//Helper Functions
//this will return a finite number 
...
//this will parse the time and return the am or pm 
...
export const createServiceAppointment = async (req, res) => {
    try {
        ...
        //for time slot checking details 
...
export const confirmServicePayment = async (req, res) => {
    try {
        ...
        console.error("confirmServicePaymen Error", err);
        return res.status(500).json({ success: false, message: "Server error" });
    }
}
...
//to get the statistics of the service appointment
export const getServiceAppointmentStats = async (req, res) => {
    try {
        const services = await Service.aggregate([
            ...
            { $sort: { createdAt: -1 } },
        ]);  //i it will you total appointments, completed appontments 
        //cancelappoinments and total earninga done by them 
        return res.json({
            success: true,
            services,
...
//to get appointment fot the patient 
export const getServiceAppointmentByPatient = async (req, res) => {
    try {
        ...
        const filter = {};
        if (resolveCreatedBy)
            filter.createdBy = resolveCreatedBy;
        if (mobile) filter.mobile = mobile;

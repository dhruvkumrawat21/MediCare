...
// backend url
const DEFAULT_HOST = "https://medicare-backend-xfn2.onrender.com/api".replace(/\/$/, "");
...
//   for clerk
  ...
  //validating the 10 digit number 
...
//check is form calid for all field are filled or not 
  const isFormValid = () => getClientMissingFields().length === 0;

  //fetching the seviced from the server side
  useEffect(() => {
    let mounted = true;
  ...
  //if no service is found with the partcular id 
  if (!service) {
    return (
      <div className={serviceDetailStyles.loadingContainer}>
        <div className={serviceDetailStyles.loadingCard}>

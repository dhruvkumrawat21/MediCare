    ...
    <>
        <nav className={navbarStylesDr.navContainer}>
                    ...
                    {navItems.map(({name,to,Icon}) => (
                        ...
                        >
                            <span className={navbarStylesDr.linkContent}>
                                ...
                                <span className={navbarStylesDr.linkText}>
                                    {name}
                                </span>
                            </span>
                        </NavLink>
            ...
            <div className={navbarStylesDr.rightActions}>
                ...
                {/* toggle menu fro small screen */}
                <button className={navbarStylesDr.hamburgerButtonMd} onClick={()=> setOpen((s) => !s)}>
                    {open ? <X size={20}/> : <Menu size={20}/>}
                </button>   

                <button className={navbarStylesDr.hamburgerButtonLg} onClick={()=> setOpen((s) => !s)}>
                    {open ? <X size={20}/> : <Menu size={20}/>}
                </button>
            </div>
        </nav>

...
// import { ChevronRight, Link, Medal, MousePointer2Off, Search } from "lucide-react";
...
const DoctorsPage = () => {
    ...
    //to load doctors coming from server side 
    useEffect(() => {
        let mounted = true;
        async function load() {
        setLoading(true);
    ...
    //only display the 8 doctors then will show you the toggle button
    const displayedDoctors = showAll
        ? filteredDoctors
        : filteredDoctors.slice(0, 8);
    //to retry the load from server 
    const retry = async () => {
        setLoading(true);
        setError("");
        try {
            ...
            <div className={doctorsPageStyles.wrapper}>
                <div className={doctorsPageStyles.headerContainer}>
                    ...
                    <p className={doctorsPageStyles.headerSubtitle}>
                        Find your ideal dcotor by name or specialization 
                    </p>
                </div>

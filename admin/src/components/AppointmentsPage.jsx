...
//helper functions 
//this function will return the  date as 22 jain 2026
...
//this funtion will take a slot and will return the date and time from that 
...
{
    const isAdmin = true;   //it will check if admin is loggedin and iss Major Admin for response send by him

    const [appointments, setAppointments] = useState([]);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);

    const [query, setQuery] = useState("");   //for filteration
    const [filterDate, setFilterDate] = useState("");
    const [filterSpeciality, setFilterSpeciality] = useState("all");
    const [showAll, setShowAll] = useState(false);

    //to fetch all the appointments coming from the server
    useEffect(() => 
    {
        async function load() 
        {
        ...
        try {
            ...
            // Agar data khud array hai, toh usko use karein, warna data.appointments ko check karein
            const appointmentsArray = Array.isArray(data) ? data : (data?.appointments || data?.appointements || []);
            const items = appointmentsArray.map((a) => {
            const doctorName =
    ...
    //computes the availabilities from fetched appointments 
    ...
    //filtering by the speciality, date and query 
    ...
    //this will sort the filtered by the datetime in decreasing order
    ...
    //displaying all the sorted in a descending order
    const displayed = useMemo(
        () => (showAll ? sortedFiltered : sortedFiltered.slice(0, 8)),
        [sortedFiltered, showAll]
    );  

    //if admin wants to cancel the appointments     
    async function adminCancelAppointment(id) 
    {
        ...
        {
            //ignore any error if occured here
        }
        }
    }

...
const AddService = () => {
    ...
    //compute the dates for the selected month/year  (respectinfg the month length)
    ...
    //preventing  the user to select the previous year/mobile/ date  from today
    useEffect(() => {
    ...
    //to fetch the services when in editing state 
    useEffect(() => 
    {
        let mounted = true;
    ...
    }, [serviceId, API_BASE]);  //pre fecth that particular service if present already on a server side  
    ...
    function handleImageChange(e) {
        ...
        //remove the existing imahe if user choose a new file 
        setRemoveImage(false);
        setHasExistingImage(false);
    }
    ...
    //to resting the form to a initial value after submitting the form 
    function resetForm() {
        if (imagePreview && imagePreview.startsWith("blob:")) {
        try {
            URL.revokeObjectURL(imagePreview);
    ...
    //to show toast for 3.5 seconds 
    function showToast(type, title, message) {
        setToast({ type, title, message });
        setTimeout(() => setToast(null), 3500);
    }

    //converts selected 12 hrs components to a date object 
    ...
    //prevent the user to select the previous date and time
    ...
    // removing the slot using a index 
    ...
    // validating each of the field filled by the user
    function validate() {
        const newErrors = {};
    ...
    //submit function for creation aor update 
    ...
    return (
        <div className={addServiceStyles.container.main}>
            ...
            <form onSubmit={handleSubmit} className={addServiceStyles.container.form}>
                <div className=" flex felx-col sm:flex-row items-start sm:items-center justify-between mb-6 sm:mb-8 gap-4">
                    <div>

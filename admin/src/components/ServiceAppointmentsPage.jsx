...
//gives the date in format of dd-mm-yyyy
...
//return the hour minute seconds ans also am or pm 
...
//it will dislpay the time and also am or pm 
function formatTimeDisplay(a) {
  return `${formatTwo(a.hour)}:${formatTwo(a.minute)} ${a.ampm}`;
}


//it will act as a small component 
//gives the status badge
...
//for status select (small component) either status in confirmend pending or completed or etc 
...
//to get todays iso   ex:- yyyy-mm-dd
...
//it will show the first latest upcoming date 
...
function RescheduleButton({ appointment, onReschedule, disabled }) {
  ...
  // to save the appointment afster editing 
  function save() {
    if (!date || !time) return;
...
const ServiceAppointmentsPage = () => {
  ...
  //to cancel the particular appointment using a admin panel 
  ...
  //display the list according to the timestamp
  //sort the date that is upcoming date first 
              ...
              <div className="flex items-center gap-2 relative w-full">
                ...
                {search ? (
                  <button 
                    ...
                    onClick={() => setSearch("")}
                  >
                      <XIcon className={serviceAppointmentsStyles.clearSearchButton}/>
                  </button>
                ) : null}
